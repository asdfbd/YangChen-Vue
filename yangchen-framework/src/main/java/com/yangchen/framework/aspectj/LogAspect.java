...
import com.yangchen.common.enums.BusinessStatus;
...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            if (e != null) {
                ...
                operLog.setErrorMsg(StrUtil.sub(Convert.toStr(e.getMessage(), ExceptionUtil.getMessage(e)), 0, 2000));
            }
            // 设置方法名称
    ...
    private void setRequestValue(JoinPoint joinPoint, SysOperLog operLog, String[] excludeParamNames) throws Exception {
        ...
        if (cn.hutool.core.map.MapUtil.isEmpty(paramsMap) && StrUtil.equalsAny(requestMethod, HttpMethod.PUT.name(), HttpMethod.POST.name(), HttpMethod.DELETE.name())) {
            String params = argsArrayToString(joinPoint.getArgs(), excludeParamNames);
            operLog.setOperParam(params);
        } else {
            operLog.setOperParam(StrUtil.sub(JsonUtils.toJsonStringExclude(paramsMap, ArrayUtils.addAll(EXCLUDE_PROPERTIES, excludeParamNames)), 0, PARAM_MAX_LENGTH));
        }
    }
        ...
        if (paramsArray != null && paramsArray.length > 0) {
            for (Object o : paramsArray) {
                if (ObjectUtil.isNotNull(o) && !isFilterObject(o)) {
                    try {
                        String jsonObj = JsonUtils.toJsonStringExclude(o, ArrayUtils.addAll(EXCLUDE_PROPERTIES, excludeParamNames));
