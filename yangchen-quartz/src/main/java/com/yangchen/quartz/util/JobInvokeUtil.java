...
public class JobInvokeUtil {
    ...
    public static List<Object[]> getMethodParams(String invokeTarget) {
        String methodStr = substringBetweenLast(invokeTarget, "(", ")");
        ...
        for (int i = 0; i < methodParams.length; i++) {
            ...
            // long长整形，以L结尾
            else if (org.apache.commons.lang3.StringUtils.endsWith(str, "L")) {
                classs.add(new Object[]{Long.valueOf(StrUtil.sub(str, 0, str.length() - 1)), Long.class});
            }
            // double浮点类型，以D结尾
            else if (org.apache.commons.lang3.StringUtils.endsWith(str, "D")) {
    ...
    private static String substringBetweenLast(String value, String open, String close) {
        int end = value.lastIndexOf(close);
        if (end < 0) return null;
        int start = value.lastIndexOf(open, end - 1);
        return start < 0 ? null : value.substring(start + open.length(), end);
    }
