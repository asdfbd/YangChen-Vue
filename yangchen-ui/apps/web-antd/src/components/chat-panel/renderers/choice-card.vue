...
const emit = defineEmits<{
  'ui-action': [
    payload: {
      action: 'submit';
      values: {choiceLabel: string; choiceValue: string};
    },
  ];
}>();
...
<template>
  <section class="choice-card">
    ...
    <div class="choice-card__content">
      ...
      <label class="choice-card__label">
        <span>请选择</span>
        <ASelect
          :value="selectedValue || undefined"
          :options="selectOptions"
          :placeholder="placeholder"
          :disabled="disabled || submitted || !options.length"
          @change="selectOption"
        />
      </label>
      ...
      <p v-if="submitted" class="choice-card__selected">
        <IconifyIcon icon="lucide:check"/>已选择「{{ selectedOption?.label }}」，正在继续处理
      </p>
      <p v-else class="choice-card__hint">选择后将立即发送；也可以直接在下方输入补充条件。</p>
    </div>
  </section>
</template>
