...
export const useTabbarStore = defineStore('core-tabbar', {
  actions: {
    ...
    /**
     * 清理由会话持久化恢复、但当前路由表中已不存在的标签页。
     * 菜单或页面被移除后，旧标签不应继续显示国际化 key 或跳转到 404。
     */
    async pruneInvalidTabs(router: Router) {
      const validTabs = this.tabs.filter((tab) => {
        if (typeof tab.name === 'string' && router.hasRoute(tab.name)) {
          return true;
        }

        const target = router.resolve(tab.fullPath || tab.path);
        return target.matched.some((route) => route.name !== 'FallbackNotFound');
      });
      ...
      this.tabs = validTabs;
      if (isVisitHistory()) {
        this.visitHistory.retain(validTabs.map((tab) => getTabKeyFromTab(tab)));
      }
      await this.updateCacheTabs();
    },
    /**
