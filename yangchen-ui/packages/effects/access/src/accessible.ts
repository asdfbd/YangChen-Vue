...
async function generateAccessible(
  ...
  // 兼容之前版本用法：包含子路由的节点移除 component，以免出现多层 BasicLayout。
  // 这里用 mapTree 递归处理多级父子路由（而不只是第一级）；
  // 如果你的项目已跟进本次修改，移除了所有自定义菜单首级的 BasicLayout，可以删除这段。
  mapTree(accessibleRoutes, (route) => {
    if (
      route.children &&
      route.children.length > 0 &&
      !route.meta?.noBasicLayout
    ) {
